...
N=Nd-1;
N2=N-1;
Forest_grid=Forest_grid(2:N,2:N); %We extract the exact size of Forest-grid
pause
Index=3;
...
while i<N
    ...
    while j<N
         if Forest_grid(i,j)==0 || Forest_grid(i,j)>2    
            ...
            %we caclulate the indices of all the neighbours of a cell
            ...
            if minimum<Inf
                ...
                % In the fillowing 4 if statements, the index is given to  
...
%It calsulates the second order moment which can be used to calculate the 
%radius of a cluster.
